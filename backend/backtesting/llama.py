...
class LLAMA:
    ...
    def sentiment_analysis(self,text):
        ...
        result=list(result["scores"])
        return candidate_labels[result.index(max(result))]
    
    def predict(test, model, tokenizer,X_test):
        y_pred = []
    ...
    def generate_test_prompt(self,data_point):
        return f"""
                Predict whether the closing price of Google stock will high or low based on the provided information.
                Open Price: {data_point["Open"]}
                Headline: {data_point["News Description"]}
    ...
    def generate_prompt(self,data_point):
        return f"""
                ...
                """.strip()
    def predict(self,test, model, tokenizer,X_test):
        y_pred = []
        for i in tqdm(range(len(X_test))):
            prompt = X_test.iloc[i]["text"]
            pipe = pipeline(task="text-generation",
    ...
    def llama_finetuning_general(self,df):
        ...
        for sentiment in ["Positive Sentiment","Neutral Sentiment","Negative Sentiment"]:
            try:
                train,test=train_test_split(df[df["sentiment"]==sentiment], train_size=len(df["Prediction"])//2, test_size=1, random_state=42)
        ...
        y_pred = self.predict(test, model, tokenizer,X_test)


    def llama_finetuning_stock_news(self):
        pass

df=pd.read_csv("yfiance_&_news_data.csv")
llama=LLAMA()
